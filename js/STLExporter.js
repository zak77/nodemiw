				...
				if ( object instanceof THREE.Mesh ) {
					...
					if ( geometry instanceof THREE.Geometry ) {
						...
						for ( var i = 0, l = faces.length; i < l; i ++ ) {
							...
							output += '\tfacet normal ' + vector.x.toFixed(5) + ' ' + vector.y.toFixed(5) + ' ' + vector.z.toFixed(5) + '\n';
							output += '\t\touter loop\n';
							...
							for ( var j = 0; j < 3; j ++ ) {
								...
								if (mesh.geometry.skinIndices.length == 0) {
									...
									output += '\t\t\tvertex ' + vector.x.toFixed(5) + ' ' + vector.y.toFixed(5) + ' ' + vector.z.toFixed(5) + '\n';
								} else {
									vector.copy( vertices[ vertexIndex ] ); //.applyMatrix4( matrixWorld );
									...
									output += '\t\t\tvertex ' + finalVector.x.toFixed(5) + ' ' + finalVector.y.toFixed(5) + ' ' + finalVector.z.toFixed(5) + '\n';
								}
							}
							output += '\t\tendloop\n';
							output += '\tendfacet\n';
